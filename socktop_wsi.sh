...
global pid_sid,pid_hash,pid_cli_info
...
probe socket.send
{
  ...
  if (( isinstr(execname()  , "ora_" ) == 1 || isinstr(execname()  , "oracle" ) == 1 ) && user_uint16(user_uint64(ksupga_address+ksupga_offset)+KSUSENUM)  > 0  ) {
	pid_sid[pid()] = user_uint16(user_uint64(ksupga_address+ksupga_offset)+KSUSENUM);
	pid_hash[pid()] = user_uint32(user_uint64(ksupga_address+ksupga_offset)+KSUSESQH);
	pid_cli_info[pid()] =  user_string2(user_uint64(ksupga_address+ksupga_offset)+KSUSECLI,"error");
  }		
}
...
function print_activity()
{
	...
	printf("======================= %s ========================\n", ctime(time))
	printf("------------------------------- PROCESSES -------------------------------\n")
	printf("%-5s %-5s %7s %7s %7s %7s %-4s %-8s %-15s %-8s %-15s %-15s\n",
	       "PID", "UID", "#SEND", "#RECV", "SEND_KB",
	       "RECV_KB", "PROT", "FAMILY", "COMMAND","SID","SQL_HASH","CLIENT_INFO")
	foreach ([pid, prot, fam] in sk_pid- limit max) {
		n_sk_tx = @count(sk_tx[pid, prot, fam])
		n_sk_rx = @count(sk_rx[pid, prot, fam])
		if (pid_sid[pid]) {
		printf("%-5d %-5d %7d %7d %7d %7d %-4s %-8s %-15s %-8d %-15d %-15s\n",
		       ...
		       execname[pid],pid_sid[pid],	pid_hash[pid],pid_cli_info[pid])		
		} else {
